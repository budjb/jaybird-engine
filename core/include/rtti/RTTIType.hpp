...
namespace core::rtti {
class RTTIClassType;
class RTTIFundamentalType;
class RTTIContainerType;
class RTTIArrayType;
...
class JAYBIRD_API RTTIType {
 public:
   ...
   * deleting through a pointer to RTTIType.
   */
   ...
   * no parent.
   */
  [[nodiscard]] RTTIType* parent() const noexcept;
   ...
   */
  void parent(RTTIType* parent) noexcept;
  ...
  /**
   * @brief Returns this type as an @c RTTIArrayType if its kind is @c RTTITypeKind::ARRAY, otherwise returns @code
   * nullptr@endcode.
   *
   * This avoids the need for a @c dynamic_cast in the common array-check case.
   *
   * @return A pointer to this descriptor as @code RTTIArrayType*@endcode when the kind is @c RTTITypeKind::ARRAY, or
   * @code nullptr@endcode otherwise.
   */
  [[nodiscard]] RTTIArrayType* asArray() const noexcept;
   ...
   */
  bool operator==(const RTTIType& type) const noexcept;
...
class TypedRTTIType : public TBase {
 public:
  /**
   ...
   */
  explicit TypedRTTIType(const Name& name, const RTTITypeKind kind)
   ...
   * @param name The name of the type.
   */
  explicit TypedRTTIType(const Name& name)
   ...
   */
  explicit TypedRTTIType(const Name& name, const RTTIType* inner)
